...
import { CannotDeleteUserWithRentError } from "../src/errors/cannot-delete-user-with-rent-error"
...
describe('App', () => {
    ...
    it('should not remove user with an active rent', async () => {
        const app = new App(userRepo, bikeRepo, rentRepo)
        const user = new User('Jose', 'jose@mail.com', '1234')
        await app.registerUser(user)
        const bike = new Bike('caloi mountainbike', 'mountain bike',
        ...
        await app.registerBike(bike)
        await app.rentBike(bike.id, user.email)
        await expect (app.removeUser(user.email)).rejects.toThrow(CannotDeleteUserWithRentError)
    })
