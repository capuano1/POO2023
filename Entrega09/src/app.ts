...
import { CannotDeleteUserWithRentError } from "./errors/cannot-delete-user-with-rent-error";
...
export class App {
    ...
    async removeUser(email: string): Promise<void> {
        ...
        let rents = await this.rentRepo.findOpenRentsFor(email)
        if (rents.length != 0) throw new CannotDeleteUserWithRentError()
